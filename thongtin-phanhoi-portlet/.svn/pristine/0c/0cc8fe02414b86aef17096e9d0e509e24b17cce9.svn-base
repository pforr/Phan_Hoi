...
<%
	...
	List<DATAITEM> lsHuyenQLy = DATAITEMLocalServiceUtil.getListHuyenGS(user, maTinh,  QueryUtil.ALL_POS, QueryUtil.ALL_POS);
	...
	if (Validator.isNotNull(userMapping)) {
		...
		lsHuyenDaChon =  DATAITEMLocalServiceUtil.findHuyenChonByUser(maTinh, selUserId,  QueryUtil.ALL_POS, QueryUtil.ALL_POS);
...
<aui:form action="<%= searchURL %>" method="get" name="fm2">
	...
	<aui:fieldset>
		...
		<liferay-ui:search-container rowChecker="<%= new RowChecker(renderResponse) %>" delta="<%= 100 %>" emptyResultsMessage="no-result-were-found" iteratorURL="<%= searchURL %>">
			...
			<liferay-ui:search-container-results results="<%= DATAITEMLocalServiceUtil.getListHuyenGS(user, maTinh, searchContainer.getStart(), searchContainer.getEnd()) %>"
								 total="<%= DATAITEMLocalServiceUtil.countHuyenByUser(user, maTinh) %>">
			...
			>
				...
				>
					...
					<liferay-ui:panel collapsible="<%= true %>" extended="<%= false %>" persistState="<%= true %>" title="<%= huyen.getName() %>" helpMessage="neu-khong-chon-theo-xa-mac-dinh-quan-ly-toan-huyen" >
						...
						<aui:fieldset>
							<liferay-util:include page="/html/backend/quanly_nguoidung/userhuyen/render_xa.jsp" servletContext="<%= application %>" >
				...
				%>
					<tr>
						...
						<td>
							<liferay-ui:panel collapsible="<%= true %>" extended="<%= false %>" persistState="<%= true %>" title="<%= huyen.getName() %>" helpMessage="neu-khong-chon-theo-xa-mac-dinh-quan-ly-toan-huyen" >
								<aui:fieldset>
									<liferay-util:include page="/html/backend/quanly_nguoidung/userhuyen/render_xa_sel.jsp" servletContext="<%= application %>" >
										<liferay-util:param name="maTinh" value="<%= maTinh %>" />
										<liferay-util:param name="maHuyen" value="<%= huyen.getNode2() %>" />
										<liferay-util:param name="userIdSel" value="<%= Long.toString(selUserId) %>" />
									</liferay-util:include>
