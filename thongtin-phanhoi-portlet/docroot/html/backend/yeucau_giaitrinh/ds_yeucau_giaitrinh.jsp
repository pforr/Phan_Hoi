...
if(themeDisplay.isSignedIn()){
	...
	_logds_yeucau_giaitrinh.info("--userMapping.getUserId()--"+userMapping.getUserId());
	if(DauNoiNuocConstantString.UserMapping.USER_TINH == userMapping.getUserType()
		...
		_logds_yeucau_giaitrinh.info("--maTinh--NotNull--"+maTinh);
	}
	if(Validator.isNotNull(maTinh)){
		_logds_yeucau_giaitrinh.info("-listHuyen-userMapping.getUserType()--"+userMapping.getUserType());
		if(DauNoiNuocConstantString.UserMapping.USER_GT_NUOC == userMapping.getUserType() || DauNoiNuocConstantString.UserMapping.USER_GT_VS == userMapping.getUserType()){//user cap huyen,xa va thap hon
			_logds_yeucau_giaitrinh.info("-listHuyen-USER_GT_NUOC-USER_GT_VS-"+userMapping.getUserType());
			...
			_logds_yeucau_giaitrinh.info("-listHuyen-USER_TINH-"+userMapping.getUserType());
			//listHuyen = UserTinhLocalServiceUtil.findHuyenQly(maTinh, userMapping.getUserId(), QueryUtil.ALL_POS, QueryUtil.ALL_POS);
			listHuyen = DATAITEMLocalServiceUtil.getHuyen(user, maTinh, 0l, isSupperAdmin, QueryUtil.ALL_POS, QueryUtil.ALL_POS);	
		}else{
			//listHuyen = DataItemBusiness.getInLevel2(WebKeys.DATAGROUPID_DONVIHANHCHINH, maTinh);
			_logds_yeucau_giaitrinh.info("-listHuyen-TW-"+userMapping.getUserType());
			listHuyen = DATAITEMLocalServiceUtil.getByDataGroupLevel2(WebKeys.DATAGROUPID_DONVIHANHCHINH,maTinh);
		}
	}
	if(Validator.isNotNull(maHuyen)){
		_logds_yeucau_giaitrinh.info("-listXa-userMapping.getUserType()--"+userMapping.getUserType());
		//listXa = DataItemBusiness.getInLevel3(WebKeys.DATAGROUPID_DONVIHANHCHINH, maTinh, maHuyen);	
		if(DauNoiNuocConstantString.UserMapping.USER_GT_NUOC == userMapping.getUserType() || DauNoiNuocConstantString.UserMapping.USER_GT_VS == userMapping.getUserType()){//user cap huyen,xa va thap hon
			_logds_yeucau_giaitrinh.info("-listXa-USER_GT_NUOC-USER_GT_VS-"+userMapping.getUserType());
			...
			listXa = DATAITEMLocalServiceUtil.findXaTrongHuyenChonByUser(maTinh, maHuyen,userMapping.getUserId(),  QueryUtil.ALL_POS, QueryUtil.ALL_POS);// list ra ma xa ma no quan ly
		}else{
			//listXa = DataItemBusiness.getInLevel3(WebKeys.DATAGROUPID_DONVIHANHCHINH, maTinh, maHuyen);
			_logds_yeucau_giaitrinh.info("-listXa-TW-"+userMapping.getUserType());
			listXa = DATAITEMLocalServiceUtil.getByDataGroupLevel3(WebKeys.DATAGROUPID_DONVIHANHCHINH,maTinh, maHuyen);
		}
	}
	/*else{
		listXa = DataItemBusiness.getInLevel3(WebKeys.DATAGROUPID_DONVIHANHCHINH, maTinh, maHuyen);
	...
	String[] xaArr = null;
	String[] loaiNc = {"01","02","03","04","05","06","07","08","12"}; // cac loai giai trinh nuoc
			...
			<%
				if(listYeuCauGiaiTrinh != null){
					...
					for(YeuCauGiaiTrinh ycgt : listYeuCauGiaiTrinh){
						...
						if((userMapping.getUserType() == DauNoiNuocConstantString.UserMapping.CAP_TU) || (userMapping.getUserType() == DauNoiNuocConstantString.UserMapping.USER_TINH)){
							...
							for(UserHuyen usrHuyen : lstUserHuyen){
								...
								if(um != null){
									...
									if(DauNoiNuocConstantString.UserMapping.USER_GT_NUOC == userType && findString(loaiNc, ycgt.getMaChuDe())){
										hasUserGt = ThongTinPhanHoiConstants.CoTaiKhoanGiaTrinh.CO;
										backgroundColor = "";
										_logds_yeucau_giaitrinh.info("--hasUserGt--CO--"+hasUserGt+"==");
										break;
									}else if(DauNoiNuocConstantString.UserMapping.USER_GT_VS == userType && findString(loaiVs, ycgt.getMaChuDe())){
										hasUserGt = ThongTinPhanHoiConstants.CoTaiKhoanGiaTrinh.CO;
										backgroundColor = "";
										_logds_yeucau_giaitrinh.info("--hasUserGt--CO--"+hasUserGt+"==");
										break;
