...
<%
	...
	if (Validator.isNull(userMapping)) {
		if (Validator.isNotNull(userMappingCreator) && userMappingCreator.getUserType() == 1) {
			lsTinhQuanLy = DATAITEMLocalServiceUtil.getListTinhTU(user, isSupperAdmin, QueryUtil.ALL_POS, QueryUtil.ALL_POS);
		} else if (Validator.isNotNull(userMappingCreator) && userMappingCreator.getUserType() == 6){
	...
	if (Validator.isNotNull(userMapping)) {
		...
		lsHuyenQuanLy = DATAITEMLocalServiceUtil.getListHuyenGS(selUser, maTinh, QueryUtil.ALL_POS, QueryUtil.ALL_POS);
...
<div class="wrapper-0">
	<div class="wrapper-1">
			...
			<aui:select name="selTinh" required="true"  label="<%= StringPool.BLANK %>" showEmptyOption="<%= lsTinhQuanLy.size() == 1 ? false : true %>" >
				<%
